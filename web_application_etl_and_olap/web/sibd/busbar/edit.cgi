...
class Edit:
    ...
    def process(self):
        try:
            ...
            busbar = self.get_busbar(data)
            title = f'Edit Bus Bar with ID = {busbar.id}'
            template = self.templates.get_template('busbar/edit.jinja2')
            print(template.render(title=title, busbar=busbar, website_root=self.config.website_root))
