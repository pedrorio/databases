...
class Update:
    ...
    def update_substation(self, data):
        ...
        sql_update_substation = """
                update substation
                set locality = %(locality)s, sname = %(sname)s, saddress = %(saddress)s
                where gpslat = %(gpslat)s and gpslong = %(gpslong)s;
                """
        cursor.execute(sql_update_substation, data)
        self.config.connection.commit()

    def process(self):
        try:
            data = self.get_parameters()
            self.update_substation(data)
