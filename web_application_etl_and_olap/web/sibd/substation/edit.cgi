...
class Edit:
    ...
    def get_substation(self, data):
        ...
        sql = """
                ...
                from substation
                where gpslat = %(gpslat)s and gpslong = %(gpslong)s;
                """
    ...
    def process(self):
        try:
            ...
            substation = self.get_substation(data)
            title = f'Edit Substation with Latitude = {substation.gpslat} and Longitude = {substation.gpslong}'
            template = self.templates.get_template('substation/edit.jinja2')
            print(template.render(title=title, substation=substation, website_root=self.config.website_root))
