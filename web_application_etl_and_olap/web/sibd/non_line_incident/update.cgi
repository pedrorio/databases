...
class Update:
    ...
    def update_incident(self, data, cursor):
        sql_update_incident = """
                        update incident
                        set instant = %(instant)s, id = %(id)s, description = %(description)s, severity = %(severity)s 
                        where id = %(id)s and instant = %(instant)s;
                        """
        cursor.execute(sql_update_incident, data)

    def update_non_line_incident(self, data):
        ...
        if self.is_line(data, cursor):
            ...
            self.update_incident(data, cursor)
    ...
    def process(self):
        try:
            ...
            self.update_non_line_incident(data)
