...
class Create:
    ...
    def create_transformer(self, data):
        ...
        sql_create_transformer = """
                insert into 
                    transformer (id, pv, sv, gpslat, gpslong, pbbid, sbbid) 
                values 
                    (%(id)s, %(pv)s, %(sv)s, %(gpslat)s, %(gpslong)s, %(pbbid)s, %(sbbid)s); 
                """
        cursor.execute(sql_create_transformer, data)
